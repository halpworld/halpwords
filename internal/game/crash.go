...
// CrashFile is where the last crash is described, in the user's folder
// (shared by every learner on the computer), so a player or teacher can
...
func Crash(what any, stack []byte) {
	...
	if err := save.Root.Write(CrashFile, []byte(report)); err != nil {
		return
	}
	if dir, err := save.Root.Dir(); err == nil {
