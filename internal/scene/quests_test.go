...
// questContext is a test context linked to a server that sends the
// Animals list and the quests in internal/link/testdata/assignments.json,
// as a sync would bring them.
func questContext(t *testing.T) *game.Context {
	t.Helper()
	useTempDir(t)
	...
	questNow = func() time.Time { return now }
	t.Cleanup(func() { questNow = time.Now })
	return ctx
}

func TestQuestOnTheTitleScreen(t *testing.T) {
	ctx := testContext(t)
	ti := NewTitle(ctx).(*Title)
	for _, it := range ti.items {
		if it == titleQuests {
			t.Fatal("Quests on the title screen of a game that isn't linked")
		}
	}
	ctx = questContext(t)
	ti = NewTitle(ctx).(*Title)
	if ti.items[0] != titleQuests || ti.sel != 0 {
	...
	if q, ok := link.Current(ti.quests, questNow()); !ok || q.ID != "asg_new_kind" {
		t.Fatalf("banner quest %q", q.ID)
	}
}

func TestQuestsScreen(t *testing.T) {
	ctx := questContext(t)
	q := NewQuests(ctx).(*Quests)
	var got []string
	...
	if !ok || p.quest == nil || p.quest.ID != "asg_new_kind" || len(p.deck.Entries()) != 3 || p.lang().Code != "fr" {
		t.Fatalf("practice %+v", p)
	}
	if p.langKeys() != "" || p.escTo() != "quests" {
	...
	if !ok || cp.setup.quest == nil || cp.setup.quest.ID != "asg_right" || setupText(cp.setup) != "Quest: Animals" {
		t.Fatalf("class pick %+v", cp)
	}
	r := newRun(ctx, cp.lang, rpg.Knight, cp.setup)
	if r.quest == nil || len(r.deck.Entries()) != 3 || r.mode != compete.Adventure || r.linkMode() != "adventure" {
	...
	if look := NewQuestsLook(ctx).(*Quests); !look.look || len(look.quests) != 6 {
		t.Fatal("look")
	}
}

func TestQuestRunSaves(t *testing.T) {
	ctx := questContext(t)
	fr, _ := words.Lookup("fr")
	r := newRun(ctx, fr, rpg.Rogue, runSetup{mode: compete.Adventure, quest: &questRun{ID: "asg_right", List: "lst_animals", Title: "Animals"}})
	l := r.floor(1)
	...
	if s.run.quest == nil || s.run.quest.ID != "asg_right" || len(s.run.deck.Entries()) != 3 {
		t.Fatalf("loaded %+v", s.run.quest)
	}
	// Without its list, the save explains.
	ctx.Lists = ctx.Lists[:len(ctx.Lists)-1]
	if _, err := decodeSave(ctx, data); err == nil || !strings.Contains(err.Error(), "Animals quest") {
		t.Fatalf("no list: %v", err)
	}
}

func TestDirectorIsToldTheQuest(t *testing.T) {
	ctx := questContext(t)
	fr, _ := words.Lookup("fr")
	// A quest run: the run's words are the quest's.
	r := newRun(ctx, fr, rpg.Knight, runSetup{mode: compete.Adventure, quest: &questRun{ID: "asg_right", List: "lst_animals", Title: "Animals"}})
	if name, ws := r.directorQuest(); name != "Animals" || ws != nil {
	...
	name, ws := r.directorQuest()
	if name != "Animals" || len(ws) != 3 {
		t.Fatalf("adventure: %q %v", name, ws)
	}
	// Not in another language.
	la, _ := words.Lookup("la")
	if name, _ := newRun(ctx, la, rpg.Knight, runSetup{mode: compete.Adventure}).directorQuest(); name != "" {
