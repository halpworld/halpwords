...
// Blank hides some letters of s behind underscores for a fill-in puzzle. The
// first letter of each word and all punctuation stay visible, and at least
// one letter is always hidden when s has any to hide.
func Blank(s string, rng *rand.Rand) string {
	rs := []rune(s)
	var can []int
	start := true
	for i, r := range rs {
	...
	n := max(1, (len(can)*2+2)/5) // about 40%
	for _, i := range can[:n] {
		rs[i] = '_'
	}
	return string(rs)
